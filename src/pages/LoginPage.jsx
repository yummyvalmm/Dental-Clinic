        ...
        <div className="min-h-[100dvh] w-full bg-bg-body relative flex flex-col items-center justify-center p-6 overflow-hidden">
            ...
            >
                ...
                >
                    ...
                    {/* Method Toggle */}
                    <div className="flex p-1 bg-[var(--glass-bg-low)] rounded-2xl mb-8 relative">
                        {/* Sliding Background */}
                        <motion.div
                            className="absolute top-1 bottom-1 bg-[var(--glass-bg-medium)] rounded-xl shadow-sm"
                            layoutId="activeTab"
                            initial={false}
                            animate={{
                        ...
                        <button
                            ...
                            className={`flex-1 relative z-10 py-3 text-sm font-bold uppercase tracking-wider transition-colors duration-300 ${method === 'email' ? 'text-[var(--color-text-main)]' : 'text-[var(--color-text-muted)]'}`}
                        >
                            Email
                        </button>
                        <button
                            onClick={() => setMethod('phone')}
                            className={`flex-1 relative z-10 py-3 text-sm font-bold uppercase tracking-wider transition-colors duration-300 ${method === 'phone' ? 'text-[var(--color-text-main)]' : 'text-[var(--color-text-muted)]'}`}
                        >
                            Phone
                        </button>
                    </div>
                    ...
                    <div className="space-y-6">
                        <AnimatePresence mode="wait">
                            ...
                            >
                                {method === 'email' ? (
                                        ...
                                        <div className="group">
                                            ...
                                            <div className="relative">
                                                <input
                                                    ...
                                                    className="w-full bg-[var(--glass-bg-low)] border border-[var(--glass-border)] rounded-2xl px-6 py-4 text-[var(--color-text-main)] placeholder-[var(--color-text-muted)] focus:outline-none focus:border-accent/50 focus:bg-[var(--glass-bg-medium)] transition-all font-medium"
                                                />
                                                <Mail className="absolute right-5 top-1/2 -translate-y-1/2 text-[var(--color-text-muted)]" size={20} />
                                            </div>
                                        </div>
                                    </div>
                                ) : (
                                    <div className="space-y-4">
                                        <div className="group">
                                            <label className="block text-xs uppercase tracking-widest text-[var(--color-text-muted)] mb-2 pl-4">Phone Number</label>
                                            <div className="relative">
                                                <input
                                                    type="tel"
                                                    ...
                                                    className="w-full bg-[var(--glass-bg-low)] border border-[var(--glass-border)] rounded-2xl px-6 py-4 text-[var(--color-text-main)] placeholder-[var(--color-text-muted)] focus:outline-none focus:border-accent/50 focus:bg-[var(--glass-bg-medium)] transition-all font-medium"
                                                />
                                                <Phone className="absolute right-5 top-1/2 -translate-y-1/2 text-[var(--color-text-muted)]" size={20} />
                                            </div>
                                        </div>
                                    </div>
                                )}
                        ...
                        <button
                            onClick={handleAuth}
                            className="w-full btn-liquid rounded-2xl py-4 text-[var(--color-primary-inverse)] font-bold tracking-wide flex items-center justify-center gap-2 group mt-4 text-blue-900 border border-white/40"
                        >
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <button className="flex items-center justify-center gap-2 px-4 py-3 bg-[var(--glass-bg-low)] hover:bg-[var(--glass-bg-medium)] border border-[var(--glass-border)] rounded-xl text-[var(--color-text-main)] font-medium transition-all group">

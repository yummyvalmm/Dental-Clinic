    ...
    return (
        <div className="fixed bottom-[calc(1rem+env(safe-area-inset-bottom))] left-4 right-4 z-50 lg:hidden pointer-events-none">
            <GlassSurface
                ...
                className="rounded-full px-6 py-4 flex justify-between items-center w-full pointer-events-auto"
                intensity="medium"
            >
                {/* Home */}
                <NavLink to="/" aria-label="Home" className="relative flex-1 flex flex-col items-center gap-1 group z-10">
                    {({ isActive }) => (
                        <motion.div
                            whileTap={{ scale: 0.9 }}
                ...
                {/* Visit */}
                <NavLink to="/visit" aria-label="Visit Clinic" className="relative flex-1 flex flex-col items-center gap-1 group z-10">
                    {({ isActive }) => (
                        <motion.div
                            whileTap={{ scale: 0.9 }}
                            onTapStart={handleTap}
                ...
                {/* Hotline (Now a Page) */}
                <NavLink to="/hotline" aria-label="Emergency Hotline" className="relative flex-1 flex flex-col items-center gap-1 group z-10">
                    {({ isActive }) => (
                        <motion.div
                            whileTap={{ scale: 0.9 }}
                            onTapStart={handleTap}
                ...
                {/* Booking */}
                <NavLink to="/book" aria-label="Book Appointment" className="relative flex-1 flex flex-col items-center gap-1 group z-10">
                    {({ isActive }) => (
                        <motion.div
                            whileTap={{ scale: 0.9 }}
                            onTapStart={handleTap}
                        ...
                        >
                            ...
                            <span className={`text-[9px] font-bold uppercase tracking-wider transition-colors duration-300 ${isActive ? 'text-white' : 'text-white/40'}`}>Book</span>
                        </motion.div>
                    )}
                </NavLink>
